// Domain requests against the web app's headless API.
//
// Transport lives in transport.ts — base URL, deadlines, auth headers, JSON
// parsing and the normalized error. This file is what the screens import, and
// ApiError is re-exported here so a caller that only ever wanted a request
// does not have to know where the transport moved to.
...
import { ApiError, BACKGROUND_TIMEOUT_MS, BASE_URL, USER_AGENT, authed, fetchOnce, get, withDeadline } from './transport';
