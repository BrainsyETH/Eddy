...
- **Pricing (shipped Jul 2026):** annual primary **$19.99/yr** + secondary **$1.99/mo** + **7-day free trial**. Optional non-renewing "Season Pass" later. ⚠️ The monthly tier was originally specified as *deliberately worse value* to steer buyers to annual; at $1.99 it no longer is (12 months = $23.88, a 1.19x premium vs 2.4x before), and a four-month season on monthly costs $7.96 — 40% of the annual price. On a seasonal product that makes May-to-September subscribing the rational choice. Revisit before launch, or expect the annual-first retention model below to describe a smaller share of buyers than it assumes.
...
- ⚠️ **As of Aug 2026 the offline river download was REMOVED, not re-tiered.** It shipped as Mapbox tile packs only, while the cached geometry/access/hazards half of "Download a river" (below) shipped free and automatic for every river on every launch. So the paid feature sold the basemap picture and the giveaway did the work: download a river, go into airplane mode, relaunch, and everything still worked. The stronger version — an on-device float estimate from the cached river mile and last reading — was scoped and is feasible, but was judged not worth the maintenance surface for a one-person team. **Eddy's written read is now the only entitlement gate in the app.** The free-tier offline behaviour described under "Offline + location are one system" is unaffected and still ships.
...
**Pricing validation:** use RevenueCat Experiments to A/B price (test upward from the shipped $19.99 — e.g. $19.99/$24.99/$29.99), trial length, and season-pass-vs-annual live — no app update. Price-test the monthly tier too: at $1.99 it is the cheapest path through a whole season, so its take-rate is the number that decides whether annual-first holds. Price-test the contextual (notify-me) paywall specifically. (The founding-floater launch price this section used to propose was $19.99 — that is now the list price, so seeding reviews via the outfitter community needs a different lever: a longer trial, or a free founding year for partner outfitters.) Instrument free-tier signals (notify-me tap, float built, offline tried) as the conversion funnel.
...
**Economics:** Apple **Small Business Program → 15%** (enroll) — ~$17 net on a $19.99 annual sub (~$1.69/mo net on monthly), **~85% gross margin** (USGS free; Mapbox/OpenWeather cacheable; map styles + PMTiles cheap to self-host; push cheap). Viability hinges on **retention** (annual hedges seasonal churn) + **CAC near-organic** (widget/outfitter channel). Legal: App Privacy labels (precise location/photos/purchases), keep analytics first-party to skip ATT, auto-renew disclosures + updated Terms/EULA/Privacy for paid+location+UGC, UGC moderation/report path, in-app account deletion (Profile), refunds via RevenueCat webhook → revoke entitlement.
...
**TAM:** ~300–500k annual Ozark floaters → the addressable unit is the **trip captain** (~1 per group of 4–6): **~60–120k** people; they're exactly who outfitter widgets/QR reach. **Unit math:** net ARPU ≈ $17 ($19.99 − Apple 15%; less for anyone on monthly, and a four-month monthly season nets only ~$6.80); run costs ≈ $3–4k/yr; marginal cost ≈ $0. **Milestones:** ~180–235 subs = pays for itself; 1,000 = $17k/yr; 3,000 = $51k/yr. **Year-1 honest read:** 5–15k realistic installs × 2–5% conversion = **100–750 subs ($1.7k–$12.8k)** — validation that covers costs, not income; the case rests on compounding (season-2 installs on season-1 reviews, coverage growth, predictive alerts, future outfitter B2B leg). **Installs, not price, are the dominant variable** → the widget/outfitter channel outranks any pricing decision.
...
**External risk — USGS legacy decommission (Q1 2027; degradation authorized from Aug 2026 — inside the build window):** ⚠️ **LARGELY RESOLVED, Aug 2026.** The claim below that percentiles were "legacy-only (no confirmed modern equivalent)" was true when written and is not any more — USGS ships a **Statistics API** (`api.waterdata.usgs.gov/statistics/v0/observationNormals`), which is not an OGC collection and so was missed. Every legacy call site has been repointed: percentiles to the Statistics API, the point-in-time photo backfill to `continuous`/`daily`, and the three importer scripts to `monitoring-locations`/`latest-continuous`. `src/lib/usgs/no-legacy-urls.test.ts` fails the build if `waterservices.usgs.gov` is named outside `flow-providers/usgs-legacy.ts`.

Two things remain: **burn in `USGS_FLOW_API=modern-only`** (preview → production, ideally through a high-water event) and then delete the fallback branches. One behaviour change is deliberate and user-visible — the modern API populates `p90`, which the legacy service always returned empty, so `upperAnchor()` now interpolates p75→p90 instead of p75→p95. Measured across five curated gauges: underlying statistics agree to ~0.1%, but **4.5% of readings change flow band, 95% of them "Higher" → "Much higher"**. Reproduce with `scripts/compare-usgs-percentiles.ts` — only possible while the legacy service still answers.
