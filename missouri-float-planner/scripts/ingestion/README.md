...
| 2 | "Use MDC/outfitter floatability + NWS as the calibration sources." | The workable calibration source turned out to be **`rivers.moherp.org` OBSERVED community trip ratings** (real trips logged at a known discharge). moherp **ESTIMATED** values and USGS percentiles were **wrong** — Annapolis estimated "Good" 536 cfs vs observed 180 cfs (real trips floated Good at 189/192). | **moherp OBSERVED = trusted (accuracy-approved). moherp ESTIMATED + USGS percentiles = REJECTED as thresholds.** Grow Eddy's own observed tier via `community_reports` over time. **Amended 2026-09-01:** (a) **owner local knowledge joins the trusted tier**, provided the migration using it records the date and the value asked for — `20260901180642` does this for both floors it moves. (b) Percentiles stay rejected as the *source* of a threshold. They may identify a **likely contradiction** only where independent observed, outfitter, or owner evidence already establishes that the reach is **normally floatable during the measured season** — and they may never supply the replacement value. Without that independent evidence the argument is invalid, not merely weak: on an intermittent or rain-dependent reach a median summer day *should* grade "too low". The upper Jacks Fork (`07065200`) is the standing counterexample. (c) **Unreconciled, needs an owner call:** `20260803170000` *did* place floor lines on percentiles (`too_low ≈ p5`, `low ≈ p25`) on Current @ Doniphan and Black @ Poplar Bluff, which this row forbids. Those two ladders are live in production. Either re-anchor them on an observed/owner source or record the exception deliberately. (d) `docs/RIVER_SCALING_PLAYBOOK.md` still documents the pre-`00177` pure-percentile ladder method end to end (`too_low` = p10, `optimal_min` = p25, …). It is **stale**; this row is authoritative. |
...
| 5 | (One Supabase project assumed.) | Data was written to a **legacy project** while the app read prod → the geography admin showed a stale, smaller river set. | **Always `export EXPECTED_SUPABASE_REF=ilefwfpvphadsbptiaur` before any `--apply`/`--write`.** Now enforced, not optional: TS scripts build clients via `scripts/lib/db.ts`, which prints the target ref and **refuses any write until the pin is set and matches** (`preload-dossier-access-points.py` keeps its own honor-the-pin check). |
