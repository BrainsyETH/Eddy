// shared/flood-stage.ts
// NWS flood stages — the THIRD visual language, and the last one.
//
// Read the headers of condition-system.ts and flow-band.ts first. Eddy already
// keeps two vocabularies deliberately apart:
//
...
//
// HERE rather than in eddy-ios/src/theme because both platforms draw these
// lines. The violet and the four-category ramp lived only on the phone while
// the website drew nothing — so the same station could show an official flood
// category on one surface and a bare chart on the other. shared/ is
// @eddy/conditions, which eddy-ios consumes as a file: dependency; the old
// eddy-ios/src/theme/floodStage.ts is now a re-export of this module.
//
