-- supabase/migrations/00166_flood_override_unit_independent.sql
--
-- Make the NWS flood-stage "Dangerous" override work regardless of threshold_unit.
--
-- get_river_condition / get_river_condition_segment classify against level_* in the
-- gauge's threshold_unit (ft or cfs). NWS flood stages are published in FEET only,
-- so once a gauge moves to CFS the official flood line would stop firing. Because
-- gauge_readings stores BOTH gauge_height_ft and discharge_cfs, we anchor Dangerous
-- to the stored gauge height vs flood_stage_ft (an `is_flood` flag), checked BEFORE
-- the unit-based bands — the exact mirror of src/lib/usgs/mo-statewide-data.ts
-- classifyStageFromThresholds. Everything else is verbatim from 00150.
