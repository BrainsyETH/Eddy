-- APPLIED to production 2026-08-04 as 20260804181951.
--
-- ── TRUNCATE, which the previous version did not look for ───────────────
--
-- Printing what was left on public.feedback after the write revoke showed the
-- next gap immediately: anon and authenticated still held TRUNCATE.
--
-- It was never in the checked set, and it is the one privilege here with no
-- second line behind it: Postgres does NOT apply row-level security to
-- TRUNCATE. For INSERT/UPDATE/DELETE the grant was redundant because RLS
-- refused them anyway. For TRUNCATE, RLS refuses nothing.
--
-- ── Proportion, so nobody reads this as an open door ────────────────────
--
-- This is Supabase's stock GRANT ALL ON ALL TABLES IN SCHEMA public TO anon,
-- authenticated — 56 of 61 public tables carry it, so it is not a mistake
-- specific to feedback. It is also not reachable with the publishable key:
-- PostgREST has no TRUNCATE verb, so exploiting it needs a direct Postgres
-- connection as anon, which needs the database password rather than the JWT.
--
-- Revoked on feedback, which is the table this audit is about, and the
-- invariant now looks for it. The other 55 are a separate decision with an
-- owner, per docs/legacy-schema-security-audit.md, deliberately not swept here.

revoke truncate on public.feedback from anon, authenticated;
revoke truncate on public.feedback from public;
...
begin
    ...
    return query select
        ...
        v_count = 0,
        case when v_count = 0 then 'no write or truncate grants to anon, authenticated or PUBLIC'
