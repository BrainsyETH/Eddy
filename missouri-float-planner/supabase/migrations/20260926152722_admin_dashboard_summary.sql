...
BEGIN
  CASE p_key
    ...
    WHEN 'versions' THEN query_text := $query$SELECT coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) FROM (WITH versions AS (SELECT coalesce(app_version,'Unknown') name,count(*) count FROM public.device_tokens WHERE platform='ios' AND last_seen_at >= $1-interval '30 days' GROUP BY app_version), ranked AS (SELECT *,row_number() OVER(ORDER BY count DESC,name) rank FROM versions) SELECT CASE WHEN rank<=19 THEN name ELSE 'Other versions' END name,sum(count) count FROM ranked GROUP BY CASE WHEN rank<=19 THEN name ELSE 'Other versions' END ORDER BY count DESC,name) t$query$;
