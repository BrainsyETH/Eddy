-- 00150_good_floor_partial_ladder.sql
-- Fix: gauges with a partial floatability ladder (only level_optimal_min set,
-- no level_low / level_optimal_max) always classified as 'too_low', at ANY
-- reading. A healthy value passed every band above and hit the final ELSE.
--
-- These are the moherp "Good begins at X" ratings: Gasconade/Jerome (400 cfs,
-- selected by get_river_condition_segment for mid-reach floats) and
-- Black/Annapolis (180 cfs, Black's primary gauge — will drive the list badge
-- once Black is activated).
--
-- Fix mirrors src/lib/conditions.ts computeCondition: the 'good' floor is
-- COALESCE(level_low, level_optimal_min), so a reading at/above where the
-- optimal band begins reads 'good' instead of falling through. Full ladders
-- (level_low present) are unchanged — COALESCE returns level_low. Only the
-- 'good' branch of get_river_condition (00103) and
-- get_river_condition_segment (00098) changes; every other branch is verbatim.
