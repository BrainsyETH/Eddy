-- 00164_harden_river_validation.sql
--
-- Two safety-threshold changes discovered auditing the first-wave river
-- ingestions (Buffalo/Bourbeuse/Gasconade/St. Francis/Black):
--
-- A. HARDEN validate_river_data(). The prior version only errored when a primary
--    gauge had ZERO thresholds or when levels were out of order. It never noticed
--    a primary that was *missing the top or bottom of its ladder* — so three
--    real gaps shipped "green":
--      - St. Francis & Gasconade had no level_dangerous → computeCondition()
--        skips the dangerous branch entirely (no flood-stage fallback), so the
--        badge caps at "High" no matter how big the river gets. On the St.
--        Francis (MO's only whitewater river) that means it could never warn
--        floaters off. SAFETY-RELEVANT.
--      - Black has optimal_min but no optimal_max → the "flowing/ideal" band
--        needs both bounds, so the whole floatable range collapses to "Good".
--      - Several primaries lack too_low → the badge can never show "Too Low".
--    These are now WARNINGS (not errors: the affected rivers are intentionally
--    live). Warnings make the gaps visible in /admin instead of silent.
--
-- B. St. Francis dangerous anchor. See dossiers/st-francis.json upper-whitewater.
--    The gauge legend's ">6 ft Above Recommended" is the do-not-float ceiling;
--    it is anchored here rather than the NWS ROZM7 minor-flood stage (15 ft =
--    valley flooding, ~9 ft above the paddler ceiling — the wrong basis for a
--    whitewater reach, which would let an 8–14 ft torrent badge merely "High").

-- ---------------------------------------------------------------------------
-- A. validate_river_data() — 00147 body + three new primary-ladder warnings.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- B. St. Francis dangerous anchor (idempotent).
-- ---------------------------------------------------------------------------
UPDATE river_gauges rg
SET level_dangerous = 6,
    level_high = NULL,
