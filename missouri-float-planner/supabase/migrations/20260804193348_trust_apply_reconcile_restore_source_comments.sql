-- APPLIED to production 2026-08-04 as 20260804193348.
--
-- Re-applies trust_apply_reconcile() so the deployed body matches the source
-- file byte for byte. No logic changed.
--
-- ── What went wrong, and how it was caught ──────────────────────────────
--
-- The function had to be transcribed into an apply_migration call rather than
-- piped from the file, and the first attempt dropped the inline commentary from
-- inside the body to reduce the amount being retyped. The logic was identical;
-- the documentation was not.
--
-- Caught by comparing md5(prosrc) on production against md5 of the body in
-- 20260804193041_trust_apply_reconcile.sql: 6101 characters deployed against
-- 7727 in the file. After this migration both read e6c03f5b33c18096a84eda028f99543b.
--
-- The same check on 20260804192753 (validate_river_data) matched first time at
-- 491da3d065543ada8cf31b5fcf52ccc0 / 8797 characters, which is the only reason
-- a 207-line hand-transcribed function replacement is trustworthy at all.
--
-- ── Why the comments were worth a migration ─────────────────────────────
--
-- pg_get_functiondef() is how somebody inspects this on production, usually
-- while something is wrong and the repository is not in front of them. A body
-- that reads differently there than it does here is a small lie that costs most
-- at the worst moment — and the reasoning it dropped is specifically the part
-- explaining why first_seen_at is absent from the DO UPDATE list and why the
-- resolve clause restates classifyExisting()'s rule.
--
-- Replay-safe: 20260804193041 already carries this exact body, so applying this
-- to a fresh database is a no-op that replaces the function with itself.
