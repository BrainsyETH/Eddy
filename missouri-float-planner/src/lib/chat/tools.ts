...
const CHAT_ONLY_TOOLS: Anthropic.Tool[] = [
  {
...
WHEN TO CALL: Use for questions your database tools can't answer, including burn bans, local events, campsite reservation links, restaurant recommendations near rivers, fishing regulations, news about river closures or bridge repairs, shuttle service contact info, gear shop locations, outfitter rates/hours/booking, or any question about rivers outside the curated coverage returned by list_rivers.
...
export const EDDY_TOOLS: Anthropic.Tool[] = [
  ...AGENT_TOOLS.map(tool => ({
    ...
    input_schema: zodToJsonSchema(tool.input, { $refStrategy: 'none' }) as Anthropic.Tool.InputSchema,
  })),
  ...CHAT_ONLY_TOOLS,
];
