...
export async function rollupTelemetry() {
    ...
    for (const day of historyDays(Date.now())) {
      try {
        ...
        const { error } = await db
          .rpc('store_upstream_snapshot', { p_rows: rows })
  ...
  for (const [table, column, days] of [
    ['admin_job_runs', 'started_at', 30],
    ['upstream_usage_daily', 'day', 90],
  ] as const) {
    const { error } = await db
      .from(table)
      .delete()
      .lt(column, new Date(Date.now() - days * 86400000).toISOString())
      .abortSignal(AbortSignal.timeout(4000));
    if (error) errors++;
  }
