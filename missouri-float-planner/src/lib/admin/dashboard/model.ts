...
export interface Metric {
  ...
  href: string;
  attention: boolean;
  state: 'ok' | 'unknown' | 'not_connected';
  value: MetricValue;
  reason?: string;
}
...
export function assembleMetrics(
  raw: Record<
    string,
    { state: 'ok' | 'unknown'; value: MetricValue; reason?: string }
  >,
): Metric[] {
...
export function inboxCount(metrics: Metric[]): number | null {
  ...
  return inbox.length === 3 &&
    inbox.every((m) => m.state === 'ok' && typeof m.value === 'number')
    ? inbox.reduce((n, m) => n + Number(m.value), 0)
    : null;
}
function version(v: string): number[] | null {
...
export function versionAdoption(
  ...
  for (const row of rows) {
    total += row.count;
    const parts = version(row.name);
    if (!parts) {
      unknown += row.count;
      continue;
    }
    if (min) {
      const diff = parts.map((v, i) => v - min[i]).find((v) => v !== 0) ?? 0;
      if (diff < 0) below += row.count;
    }
    if (current && parts.every((v, i) => v === current[i])) newest += row.count;
  }
  return {
    below: min ? below : null,
    newest: current ? newest : null,
    unknown,
    total,
  };
}
