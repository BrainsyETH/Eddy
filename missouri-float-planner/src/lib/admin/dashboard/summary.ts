...
import { assembleMetrics, versionAdoption, type DashboardSummary, type Metric } from './model';
...
async function sentryMetrics(): Promise<Metric[]> {
  ...
  const projects = (process.env.SENTRY_DASHBOARD_PROJECTS ?? '').split(',').map(s=>s.trim()).filter(Boolean).slice(0,3);
  if (!token || !org || !projects.length) return [{ key:'sentry',section:'Errors',label:'Sentry',detail:'Configure a read-only dashboard token and project slugs.',href:'https://sentry.io',attention:false,state:'not_connected',value:null }];
  return Promise.all(projects.map(async project => {
    ...
    } catch { return { ...meta,state:'unknown' as const,value:null }; }
  }));
}

export const getDashboardSummary = unstable_cache(async (): Promise<DashboardSummary> => {
  let metrics: Metric[];
  try {
    const db = createAdminClient();
    const { data,error } = await db.rpc('admin_dashboard_summary',{p_now:new Date().toISOString(),p_entitlement:DEFAULT_ENTITLEMENT_ID}).abortSignal(AbortSignal.timeout(9000));
    metrics = assembleMetrics(error ? {} : data ?? {});
  } catch { metrics=assembleMetrics({}); }
  const config = metrics.find(m=>m.key==='app_config');
  const value = config?.state==='ok' ? config.value as Record<string, unknown> : null;
  const pushOff = process.env.EXPO_PUSH_ENABLED==='false' || value?.push_enabled===false;
  metrics.push({key:'effective_push',section:'Configuration',label:'Push sending disabled',detail:process.env.EXPO_PUSH_ENABLED==='false'?'EXPO_PUSH_ENABLED=false':value?'Includes environment and database switches.':'Configuration unreadable; sender uses its existing fail-open behavior.',href:'',attention:true,state:pushOff || value?'ok':'unknown',value:pushOff?1:value?0:null});
