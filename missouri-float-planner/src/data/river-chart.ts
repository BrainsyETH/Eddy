// src/data/river-chart.ts
// Hand-drawn SVG river data ported verbatim from the Stitch design bundle
// `/tmp/stitch-design/extracted/robust-river-map/project/river-data.js`.
//
// Coordinates live in the chart's stylized 1600×1000 SVG space — they are NOT
// georeferenced. Access points sit at parametric `t∈[0,1]` along their river's
// path. This is the visual instrument; the real planner at /plan?river=<slug>
// uses actual GeoJSON.

export type ChartLevel = 'bony' | 'skinny' | 'prime' | 'pushy' | 'blown';
export type ChartVerdict = 'wait' | 'float' | 'danger';

export interface ChartLevelMeta {
  label: string;
  badge: 'low' | 'flowing' | 'high' | 'danger';
  color: string;
  glow: string;
  weight: number;
  speed: number;
...
export const CHART_LEVELS: Record<ChartLevel, ChartLevelMeta> = {
...
export interface ChartAccess {
  t: number;
  name: string;
  type: 'put-in' | 'take-out' | 'both';
  difficulty: string;
  notes: string;
  lot: string;
}

export interface ChartRiver {
  id: string; // matches the river slug in our DB so click-through works
  name: string;
  blurb: string;
  path: string; // SVG path "d" attribute in 1600×1000 space
  level: ChartLevel; // fallback level if no live data is available
  gauge: { id: string; site: string; reading: string; cfs: string; trend: string };
  voice: string;
  access: ChartAccess[];
  // Where the river name label sits along the path (parametric t∈[0,1])
  labelT: number;
}

export const CHART_RIVERS: ChartRiver[] = [
  {
    id: 'current',
    name: 'Current River',
...
export const CHART_VIEWBOX = { width: 1600, height: 1000 };
