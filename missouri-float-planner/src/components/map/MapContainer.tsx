...
function ensureHillshade(map: maplibregl.Map) {
  // Anchors are the capability signal: only the curated style carries
  // them, so Positron/satellite are naturally skipped.
...
export default function MapContainer({
  ...
  useEffect(() => {
    ...
    // stale). Dark background for dark styles; hillshade on the curated
    // style (ensureHillshade no-ops elsewhere); epoch bump remounts child
    // layers that setStyle just wiped.
    map.current.on('style.load', () => {
      if (!map.current) return;
      if (MAP_STYLES[styleKeyRef.current].dark) {
        try {
