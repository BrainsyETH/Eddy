...
export default function ChatBubble() {
  ...
  useEffect(() => {
    const check = () => {
      // The bottom sheet has a specific class pattern: fixed bottom-0 ... lg:hidden
      const sheet = document.querySelector('.fixed.bottom-0.z-50.rounded-t-3xl');
      if (sheet) {
        setBottomSheetVisible(true);
        // Check if expanded (height > 200px means expanded)
    ...
    check();
    // Re-check on DOM changes (sheet appears/disappears/resizes)
  ...
  // Hide FAB when bottom sheet is expanded (user is focused on planning)
  const hideFab = bottomSheetExpanded && !isOpen;

  // Calculate FAB bottom offset — shift above collapsed bottom sheet on mobile
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          aria-label="Ask Eddy"
        >
          {/* Otter avatar with branded ring */}
      ...
      {isOpen && (
        <>
          ...
          <div
            className="fixed inset-0 bg-black/40 z-[70] lg:bg-transparent lg:pointer-events-none"
            onClick={() => setIsOpen(false)}
          />

          {/* Chat panel container */}
          <div
            className="fixed z-[80] inset-x-0 bottom-0 lg:inset-auto lg:right-4 lg:bottom-4 lg:w-[400px] lg:h-[600px] lg:rounded-2xl lg:shadow-2xl lg:border-2 lg:border-primary-200 overflow-hidden animate-slide-up lg:animate-fade-in"
