-- File: supabase/migrations/00118_dataset_uses_snapped_pois_and_camps.sql
-- Bump get_mo_surface_water_dataset() to read points_of_interest and
-- nps_campgrounds via the new location_snap columns added in 00117.
-- Falls back to raw (longitude, latitude) when the row is outside the
-- 1500 m snap window — i.e., when it's a feature that genuinely doesn't
-- belong on a riverbank.
--
-- Identical to 00115 except for the lat/lon expressions inside the POI
-- and campground subqueries.
...
AS $$
    ...
    SELECT
      ...
      (
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'slug', poi.slug,
          'type', poi.type,
          -- Snapped coords when the POI sits on (or very near) the
          -- assigned river; raw lat/lon otherwise (00117).
  ...
  campground_rows AS (
    SELECT jsonb_agg(jsonb_build_object(
      'id', c.id,
      'name', c.name,
      -- Snapped coords when within 1500 m of an active river (00117);
      -- raw lat/lon otherwise.
